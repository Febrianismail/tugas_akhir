            ...
            <form method="POST" enctype="multipart/form-data">
                <div class="card">
                    <div class="card-header">
                        <div class="card-title">PERSYARATAN REQUEST SURAT KERAMAIAN</div>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <?php
                            include '../konek.php';
                            //session_start();
                            $tampilkandatakeramaian = "SELECT * FROM t_keramaian WHERE nik=$_SESSION[nik_keramaian]";

                            // nik ganti session

                            $query = mysqli_query($konek, $tampilkandatakeramaian);
                            $datakeramaian = mysqli_fetch_array($query, MYSQLI_BOTH);

                            $nik = '';
                            $idkeramaian = '';
                            if (isset($datakeramaian)) {
                                $nik = $datakeramaian['nik'];
                                $idkeramaian = $datakeramaian['id_keramaian'];
                            }
                            ...
                            $tampilScan = "SELECT *, data_jenis_persyaratan.id as idPersyaratan, data_image.id as idImage FROM `data_image` JOIN data_jenis_persyaratan ON data_image.jenis_surat_id = data_jenis_persyaratan.id WHERE `keramaian_id`= $idkeramaian ";
                            ...
                            <div class="container mt-5">
                                ...
                                <form method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <a href="?halaman=request_sku" class="btn btn-secondary">Kembali</a>
                                    </div>
                                </form>
                            </div>
                                    ...
                                    <tbody>
                                        <?php foreach ($dataScans as $keysScans => $vScans) { ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="?halaman=scankeramaian&delete=<?= $vScans['idImage'] ?>" class="btn btn-danger">Hapus</a>
                                                </td>
                                            </tr>
                                        <?php } ?>
                                    </tbody>
                            ...
                            if (isset($_POST['tambah'])) {
                                ...
                                if (!empty($jenisPersyaratan)) {
                                    ...
                                    $querySimpanScan = "INSERT INTO `data_image` (`keramaian_id`, `jenis_surat_id`, `path_image`) VALUES ('$idkeramaian','$jenisPersyaratan', '$file_name') ";
                                    ...
                                    echo '<meta http-equiv="refresh" content="0; url=?halaman=scankeramaian">';
                                } else {
